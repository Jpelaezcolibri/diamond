...
const COMMAND_TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    description:
      "Aprueba manualmente un pedido del radar que quedo CALLADO (respuesta.salio=false en trazabilidad_radar) y le escribe al PRIVADO del colega, igual que si hubiera salido en modo automatico. NO publica en el grupo: ninguna via del radar lo hace desde el 2026-09-02. Usala cuando el admin diga 'aprueba ese', 'mandalo igual', 'publica esa propiedad', 'respondele a X' sobre un pedido que el radar no contesto solo. La aprobacion humana reemplaza el UMBRAL DE PUNTAJE (el motivo mas comun de callado, 'puntaje_bajo') — por eso sirve justo para los casos de zona vecina o puntaje limite que el admin decide que si valen la pena. NO reemplaza la calidad del dato: si es de la red de aliados, si la zona es la equivocada, si el link no abre o el sync esta viejo, te va a decir por que en vez de publicar cualquier cosa. Si hay varios pedidos callados que coinciden con la descripcion, te los va a listar para que preguntes cual.",
    input_schema: {
      type: "object",
      properties: {
