...
export default function LeadsTable({ leads, admin }: { leads: Lead[]; admin: boolean }) {
  ...
  const filtered = useMemo(() => {
    ...
    return leads.filter((l) =>
      [l.nombre, l.phone, l.property_ref_origen, l.forma_pago, l.urgencia]
        .filter(Boolean)
          ...
          <tbody>
            {filtered.map((l) => (
              <tr key={l.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50/80">
                ...
                <td className="px-4 py-3 text-slate-600">{l.forma_pago || "—"}</td>
                <td className="px-4 py-3 text-slate-600">{l.urgencia || "—"}</td>
                {admin && (
